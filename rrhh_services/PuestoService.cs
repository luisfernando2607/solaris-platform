...
public class PuestoService : IPuestoService
{
    ...
    {
        var p = await _db.Set<Puesto>()
            .Include(p => p.Departamento)
            .FirstOrDefaultAsync(p => p.Id == id);

        if (p == null)
            return ApiResponse<PuestoDto>.Fail("Puesto no encontrado");
    ...
    public async Task<ApiResponse<PuestoDto>> CrearAsync(long empresaId, CrearPuestoRequest request)
    {
        ...
        var deptExiste = await _db.Set<Departamento>().AnyAsync(d => d.Id == request.DepartamentoId && d.EmpresaId == empresaId);
        if (!deptExiste)
        ...
        var entidad = new Puesto
        {
            ...
            BandaSalarialMin = request.BandaSalarialMin,
            BandaSalarialMax = request.BandaSalarialMax,
    ...
    public async Task<ApiResponse<PuestoDto>> ActualizarAsync(long id, ActualizarPuestoRequest request)
    {
        ...
        entidad.NivelJerarquico  = request.NivelJerarquico;
        entidad.BandaSalarialMin = request.BandaSalarialMin;
        entidad.BandaSalarialMax = request.BandaSalarialMax;
    ...
    public async Task<ApiResponse> EliminarAsync(long id)
    {
        var entidad = await _db.Set<Puesto>().FindAsync(id);
        if (entidad == null)
            return ApiResponse.Fail("Puesto no encontrado");

        bool tieneEmpleados = await _db.Set<Empleado>().AnyAsync(e => e.PuestoId == id && e.Activo);
        if (tieneEmpleados)
            return ApiResponse.Fail("No se puede eliminar un puesto que tiene empleados activos asignados");
