// ══════════════════════════════════════════════════════════════════════
//  test.config.js  —  Solaris Platform · API Test Console  v4.0
//  ─────────────────────────────────────────────────────────────────────
//  ÚNICA FUENTE DE VERDAD — sincronizado con swagger (2026-03-07)
//  CRUD completo con flujos encadenados: crea → lee → edita → borra
//  v4.0: cobertura 100% — todos los 243 endpoints del swagger incluidos
//
...
const BODY_FACTORIES = {
  ...
  // Roles
  'create-rol':            ()    => ({ codigo:`ROL${Date.now()}`, nombre:`Rol Test ${Date.now()}`, descripcion:'Creado por runner', nivel:0, activo:true }),
  'update-rol':            ()    => ({ nombre:'Rol Editado Runner', descripcion:'Editado por runner', activo:true }),

  // Empresas
  'create-empresa':        (D,S) => ({ nombre:`Empresa Test ${Date.now()}`, ruc:`${Date.now()%10000000000}`, email:`emp${Date.now()}@solaris.dev`, activo:true }),
  'update-empresa':        (D,S) => ({ nombre:'Empresa Editada Runner', activo:true }),
  ...
  'create-proyecto':        (D,S) => ({ codigo:`PROY${Date.now()}`, nombre:`Proyecto Test ${Date.now()}`, descripcion:'Proyecto de prueba', empresaId:S.empresaId, tipoProyecto:1, prioridad:2, fechaInicioPlan:'2025-01-01', fechaFinPlan:'2025-12-31', presupuestoTotal:100000 }),
...
const TEST_SUITES = [
  ...
  // ══════════════════════════════════════════════════════════════════
  //  SMOKE — ¿Está viva la API?
  // ══════════════════════════════════════════════════════════════════
  { id:'smoke', icon:'🔍', title:'Smoke Tests', desc:'¿La API está viva?', tests:[
    { id:'health',        method:'GET', path:'/health',       auth:false, name:'GET /health',       desc:'Health general',    expect:{status:200} },
    ...
    { id:'test-endpoint', method:'GET', path:'/test',         auth:false, name:'GET /test',         desc:'Test endpoint',     expect:{status:[200,404]} },
  ...
  //  AUTENTICACIÓN
  // ══════════════════════════════════════════════════════════════════
  { id:'auth', icon:'🔑', title:'Autenticación', desc:'Login, registro, JWT, refresh, logout, password', tests:[
    ...
    { id:'resetear-password',   method:'POST', path:'/api/Auth/resetear-password',  auth:false, name:'POST resetear-password — token falso → 400/404', body:{token:'fake-token-runner',nuevaPassword:'Solaris789!'}, expect:{status:[400,404]}, hint:'Token inválido esperado' },
    { id:'verificar-email',     method:'GET',  path:'/api/Auth/verificar-email/fake-token-runner', auth:false, name:'GET verificar-email — token falso → 400/404', expect:{status:[400,404]}, hint:'Token de verificación inválido' },
    { id:'registro',            method:'POST', path:'/api/Auth/registro',           auth:false, name:'POST registro — nuevo usuario', bodyFn:'registro', expect:{status:[200,201,400,409]}, hint:'409 si el email ya existe; 400 si validación falla' },
  ...
  //  ROLES
  // ══════════════════════════════════════════════════════════════════
  { id:'roles', icon:'🛡️', title:'Roles', desc:'CRUD completo + módulos/permisos', tests:[
  ...
  //  EMPRESAS & SUCURSALES
  // ══════════════════════════════════════════════════════════════════
  { id:'empresas', icon:'🏢', title:'Empresas & Sucursales', desc:'CRUD empresa + sucursales', tests:[
    { id:'list-empresas',    method:'GET',    path:'/api/Empresas',                                    auth:true, name:'GET Empresas — listar',    expect:{status:200} },
    { id:'create-empresa',   method:'POST',   path:'/api/Empresas',                                    auth:true, name:'POST Empresa — crear',     bodyFn:'create-empresa', captureId:'newEmpresaId', expect:{status:[200,201,400]}, hint:'400 aceptado si RUC único ya existe' },
    { id:'get-empresa-new',  method:'GET',    path:'/api/Empresas/{DYN}',                              auth:true, name:'GET Empresa creada',       useDynamic:'newEmpresaId', skipIf:'newEmpresaId', expect:{status:200} },
    { id:'update-empresa',   method:'PUT',    path:'/api/Empresas/{DYN}',                              auth:true, name:'PUT Empresa — editar',     useDynamic:'newEmpresaId', skipIf:'newEmpresaId', bodyFn:'update-empresa', expect:{status:[200,204]} },
    { id:'get-empresa-seed', method:'GET',    path:`/api/Empresas/${SEED.empresaId}`,                  auth:true, name:'GET Empresa seed',         expect:{status:[200,404]} },
    { id:'list-sucursales',  method:'GET',    path:`/api/Empresas/${SEED.empresaId}/sucursales`,       auth:true, name:'GET sucursales seed',      expect:{status:200} },
  ...
  { id:'catalogos', icon:'🗂️', title:'Catálogos', desc:'CRUD: Países, Estados, Ciudades, Monedas, TiposId, Impuestos, FormasPago, Bancos', tests:[
    ...
    { id:'get-estado',       method:'GET',    path:'/api/catalogos/estados-provincias/{DYN}',                     auth:true, name:'GET estado creado',    useDynamic:'newEstadoId', skipIf:'newEstadoId', expect:{status:200} },
    ...
    { id:'get-ciudad',       method:'GET',    path:'/api/catalogos/ciudades/{DYN}',                        auth:true, name:'GET ciudad creada',    useDynamic:'newCiudadId', skipIf:'newCiudadId', expect:{status:200} },
    ...
    { id:'get-tipo-id',      method:'GET',    path:'/api/catalogos/tipos-identificacion/{DYN}',                     auth:true, name:'GET tipo-id creado',  useDynamic:'newTipoIdId', skipIf:'newTipoIdId', expect:{status:200} },
    ...
    { id:'get-impuesto',     method:'GET',    path:'/api/catalogos/impuestos/{DYN}',         auth:true, name:'GET impuesto creado', useDynamic:'newImpuestoId', skipIf:'newImpuestoId', expect:{status:200} },
    ...
    { id:'get-forma-pago',    method:'GET',    path:'/api/catalogos/formas-pago/{DYN}',         auth:true, name:'GET forma-pago',     useDynamic:'newFormaPagoId', skipIf:'newFormaPagoId', expect:{status:200} },
  ...
  // ══════════════════════════════════════════════════════════════════
  //  RRHH — DEPARTAMENTOS & PUESTOS
  // ══════════════════════════════════════════════════════════════════
  { id:'rrhh-base', icon:'👥', title:'RRHH — Departamentos & Puestos', desc:'CRUD departamentos y puestos', tests:[
  ...
  //  RRHH — EMPLEADOS
  // ══════════════════════════════════════════════════════════════════
  { id:'rrhh-empleados', icon:'🧑‍💼', title:'RRHH — Empleados', desc:'CRUD + historial + documentos + cambios', tests:[
    ...
    { id:'empleado-docs-get',   method:'GET',    path:'/api/rrhh/empleados/{DYN}/documentos',              auth:true, name:'GET empleado/documentos',   useDynamic:'newEmpleadoId', skipIf:'newEmpleadoId', expect:{status:200} },
    { id:'empleado-docs-post',  method:'POST',   path:'/api/rrhh/empleados/{DYN}/documentos',              auth:true, name:'POST empleado/documentos',  useDynamic:'newEmpleadoId', skipIf:'newEmpleadoId', body:{tipoDocumento:'Contrato', nombre:'Contrato Runner', url:'https://docs.solaris.dev/test.pdf'}, captureId:'newEmpleadoDocId', expect:{status:[200,201,400]}, hint:'400 aceptado si el DTO requiere campos adicionales' },
    { id:'empleado-docs-delete',method:'DELETE', path:'/api/rrhh/empleados/documentos/{DYN}',              auth:true, name:'DELETE empleado/documentos', useDynamic:'newEmpleadoDocId', skipIf:'newEmpleadoDocId', expect:{status:[200,204]} },
  ...
  { id:'rrhh-asistencia', icon:'📅', title:'RRHH — Asistencia & Ausencias', desc:'Asistencia (en desarrollo), ausencias', tests:[
    ...
    { id:'create-ausencia',     method:'POST', path:'/api/rrhh/ausencias',                              auth:true, name:'POST ausencia', bodyFn:'create-ausencia', captureId:'newAusenciaId', expect:{status:[200,201,400]}, hint:'400 aceptado — funcionalidad en desarrollo' },
  ...
  { id:'rrhh-nomina', icon:'💰', title:'RRHH — Nómina', desc:'Conceptos, períodos, roles de pago, parámetros', tests:[
    ...
    { id:'create-concepto',      method:'POST',   path:'/api/rrhh/nomina/conceptos',                                   auth:true, name:'POST concepto',             bodyFn:'create-nomina-concepto', captureId:'newConceptoId', expect:{status:[200,201,400]}, hint:'400 aceptado — funcionalidad en desarrollo' },
    ...
    { id:'create-rol-pago',      method:'POST',   path:'/api/rrhh/nomina/roles-pago',                                  auth:true, name:'POST rol-pago',             body:{periodoId:null, descripcion:`Rol Test ${Date.now()}`}, captureId:'newRolPagoId', expect:{status:[200,201,400]}, hint:'400 aceptado si periodoId es requerido' },
    { id:'get-rol-pago',         method:'GET',    path:'/api/rrhh/nomina/roles-pago/{DYN}',                            auth:true, name:'GET rol-pago creado',       useDynamic:'newRolPagoId', skipIf:'newRolPagoId', expect:{status:[200,404]} },
    { id:'calcular-rol-pago',    method:'POST',   path:'/api/rrhh/nomina/roles-pago/{DYN}/calcular',                   auth:true, name:'POST rol-pago/calcular',    useDynamic:'newRolPagoId', skipIf:'newRolPagoId', body:{}, expect:{status:[200,201,204,400]}, hint:'400 aceptado si no hay empleados asignados' },
    { id:'aprobar-rol-pago',     method:'POST',   path:'/api/rrhh/nomina/roles-pago/{DYN}/aprobar',                    auth:true, name:'POST rol-pago/aprobar',     useDynamic:'newRolPagoId', skipIf:'newRolPagoId', body:{aprobado:true}, expect:{status:[200,204,400]}, hint:'400 aceptado si estado no permite aprobación' },
    { id:'marcar-pagado',        method:'POST',   path:'/api/rrhh/nomina/roles-pago/{DYN}/marcar-pagado',              auth:true, name:'POST rol-pago/marcar-pagado', useDynamic:'newRolPagoId', skipIf:'newRolPagoId', body:{}, expect:{status:[200,204,400]}, hint:'400 aceptado si no está aprobado' },
    { id:'nomina-rol-detalle',   method:'GET',    path:'/api/rrhh/nomina/roles-pago/{DYN}/detalle',                    auth:true, name:'GET rol-pago/detalle',      useDynamic:'newRolPagoId', skipIf:'newRolPagoId', expect:{status:[200,404]} },
    { id:'nomina-rol-det-seed',  method:'GET',    path:`/api/rrhh/nomina/roles-pago/${SEED.nominaId}/detalle`,         auth:true, name:'GET rol-pago/detalle seed', expect:{status:[200,404]} },
  ...
  // ══════════════════════════════════════════════════════════════════
  //  PROYECTOS — PLANIFICACIÓN (Fases, Hitos, WBS)
  // ══════════════════════════════════════════════════════════════════
  { id:'proyectos-planificacion', icon:'🏗️', title:'Proyectos — Planificación', desc:'CRUD fases, hitos, WBS', tests:[
    ...
    { id:'get-fase',            method:'GET',    path:'/api/proy/proyectos/{DYN}/fases/{newFaseId}',              auth:true, name:'GET fase creada',       useDynamic:'newProyectoId', skipIf:'newFaseId', expect:{status:200} },
    ...
    { id:'get-hito',            method:'GET',    path:'/api/proy/proyectos/{DYN}/hitos/{newHitoId}',              auth:true, name:'GET hito creado',       useDynamic:'newProyectoId', skipIf:'newHitoId', expect:{status:200} },
    ...
    { id:'get-wbs',             method:'GET',    path:'/api/proy/proyectos/{DYN}/wbs/{newWbsId}',                 auth:true, name:'GET wbs creado',        useDynamic:'newProyectoId', skipIf:'newWbsId', expect:{status:200} },
  ...
  // ══════════════════════════════════════════════════════════════════
  //  PROYECTOS — TAREAS & CUADRILLAS
  // ══════════════════════════════════════════════════════════════════
  { id:'proyectos-tareas', icon:'✅', title:'Proyectos — Tareas & Cuadrillas', desc:'CRUD tareas + dependencias + cuadrillas', tests:[
    ...
    { id:'add-miembro',            method:'POST',   path:'/api/proy/proyectos/{DYN}/cuadrillas/{newCuadrillaId}/miembros',        auth:true, name:'POST cuadrilla/miembros',   useDynamic:'newProyectoId', skipIf:'newCuadrillaId', body:{empleadoId:SEED.empleadoId, rol:1}, expect:{status:[200,201,400,409]} },
  ...
  { id:'proyectos-presupuesto', icon:'💰', title:'Proyectos — Presupuesto & Costos', desc:'Presupuesto, partidas, centros de costo', tests:[
    ...
    { id:'get-cc',               method:'GET',    path:'/api/proy/proyectos/{DYN}/centros-costo/{newCCId}',                          auth:true, name:'GET centro-costo creado',    useDynamic:'newProyectoId', skipIf:'newCCId', expect:{status:[200,404]} },
    ...
    { id:'asignacion-cc',        method:'POST',   path:'/api/proy/proyectos/{DYN}/centros-costo/{newCCId}/asignaciones',             auth:true, name:'POST centro-costo/asignaciones', useDynamic:'newProyectoId', skipIf:'newCCId', body:{monto:1000.00, descripcion:'Asignación runner', fecha:new Date().toISOString().split('T')[0]}, expect:{status:[200,201,400]}, hint:'400 aceptado si faltan campos requeridos' },
  ...
  // ══════════════════════════════════════════════════════════════════
  //  PROYECTOS — CONTROL (Gantt, KPIs, Alertas, Reportes, Documentos)
  // ══════════════════════════════════════════════════════════════════
  { id:'proyectos-control', icon:'📊', title:'Proyectos — Control', desc:'Gantt, KPIs, alertas, reportes, documentos', tests:[
    ...
    { id:'gantt-linea-base',   method:'POST', path:`/api/proy/proyectos/${SEED.proyectoId}/gantt/linea-base`,   auth:true, name:'POST gantt/linea-base',   body:{}, expect:{status:[200,201,204,400]}, hint:'400 esperado si proyecto seed no tiene tareas' },
    ...
    { id:'marcar-alerta',      method:'PATCH', path:`/api/proy/proyectos/${SEED.proyectoId}/alertas/${SEED.alertaId}/leida`, auth:true, name:'PATCH alerta/leida',       body:{}, expect:{status:[200,204,404,500]}, hint:'500 conocido: error EF Core — pendiente fix backend' },
    ...
    { id:'add-foto-reporte',   method:'POST',   path:'/api/proy/proyectos/{DYN}/reportes/{newReporteId}/fotos',              auth:true, name:'POST reporte/fotos',       useDynamic:'newProyectoId', skipIf:'newReporteId', body:{url:'https://storage.solaris.dev/test.jpg', descripcion:'Foto runner'}, captureId:'newFotoId', expect:{status:[200,201,400]}, hint:'400 aceptado si el DTO requiere multipart' },
    { id:'delete-foto-reporte',method:'DELETE', path:'/api/proy/proyectos/{DYN}/reportes/{newReporteId}/fotos/{newFotoId}',  auth:true, name:'DELETE reporte/fotos',     useDynamic:'newProyectoId', skipIf:'newFotoId', expect:{status:[200,204,404]} },
    ...
    { id:'create-doc',         method:'POST',   path:'/api/proy/proyectos/{DYN}/documentos',                                 auth:true, name:'POST documento',           useDynamic:'newProyectoId', skipIf:'newProyectoId', body:{nombre:'Documento Runner', tipo:'Plano', url:'https://docs.solaris.dev/runner.pdf', descripcion:'Doc de prueba'}, captureId:'newDocId', expect:{status:[200,201,400]}, hint:'400 aceptado si el DTO requiere campos adicionales' },
    { id:'delete-doc',         method:'DELETE', path:'/api/proy/proyectos/{DYN}/documentos/{newDocId}',                      auth:true, name:'DELETE documento',         useDynamic:'newProyectoId', skipIf:'newDocId', expect:{status:[200,204,404]} },
  ]},
